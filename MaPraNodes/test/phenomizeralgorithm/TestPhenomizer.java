...
public class TestPhenomizer {
	...
	private void readData(int i, boolean weight, boolean pval){
		query = FileUtilitiesPhenomizer.readInQuery("../TestData/Phenomizer/Queries/query"+i+".txt");
		...
		if(!weight){
			if(!pval){
				expected_res = FileInputReader.readAllLinesFrom("../TestData/Phenomizer/ExpectedResults/NoWeightNoP/res_q"+i+".txt");
			}
			else{
				expected_res = FileInputReader.readAllLinesFrom("../TestData/Phenomizer/ExpectedResults/NoWeightP/res_q"+i+".txt");
			}
		}
		else{
			expected_res = FileInputReader.readAllLinesFrom("../TestData/Phenomizer/ExpectedResults/WeightNoP/res_q"+i+".txt");
		}
		//remove table header
...
//		for(int i=1; i<=1; i++){
	...
	public void testPhenomizer_NoWeight_PVal() {
		...
		for(int i=1; i<=10; i++){
			...
			PhenomizerDriver d = new PhenomizerDriver(query, symptoms, ksz_no_freq, ontology);
			d.setPhenomizerAlgorithm(11, true, 0, "../TestData/Phenomizer/PValues");
			LinkedList<String[]> result = d.runPhenomizer();
			for(int j=0; j<=10; j++){
				String[] elements = expected_res.get(j).split("\t");
				assertEquals("Query "+i+" result "+j+" does not match expected disease id",
	...
	public void testPhenomizer_LimitedOutput(){
		...
		expected_res=FileInputReader.readAllLinesFrom(
				"../TestData/Phenomizer/ExpectedResults/ResultSizeLimited/res_q5_noweight_nopval.txt");
		expected_res.remove(0);
		...
		expected_res=FileInputReader.readAllLinesFrom(
				"../TestData/Phenomizer/ExpectedResults/ResultSizeLimited/res_q5_noweight_pval.txt");
		expected_res.remove(0);
		
		d = new PhenomizerDriver(query, symptoms, ksz_no_freq, ontology);
		d.setPhenomizerAlgorithm(4, true, 0, "../TestData/Phenomizer/PValues");
